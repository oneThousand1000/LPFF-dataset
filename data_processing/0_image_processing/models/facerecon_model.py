...
class FaceReconModel(BaseModel):
    ...
    def compute_visuals(self):
        with torch.no_grad():
            ...
            output_vis_numpy_raw = 255. * (output_vis.detach().cpu().permute(0, 2, 3, 1).numpy() * 0.4 + self.input_img.detach().cpu().permute(0, 2, 3, 1).numpy()*0.6)
            
            if self.gt_lm is not None:
                gt_lm_numpy = self.gt_lm.cpu().numpy()
                pred_lm_numpy = self.pred_lm.detach().cpu().numpy()
                output_vis_lm = util.draw_landmarks(input_img_numpy, gt_lm_numpy, 'b')
                output_vis_lm = util.draw_landmarks(output_vis_lm, pred_lm_numpy, 'r')
            
                output_vis_numpy = np.concatenate((input_img_numpy, 
                                    output_vis_lm, output_vis_numpy_raw), axis=-2)
            else:
                output_vis_numpy = np.concatenate((input_img_numpy, 
