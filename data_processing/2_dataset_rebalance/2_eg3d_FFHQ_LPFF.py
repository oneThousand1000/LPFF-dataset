...
if __name__ == '__main__':
    ...
    thetas_lpff = thetas_all[139914:]
    phis_lpff= phis_all[139914:]
    ...
    with open(os.path.join(eg3d_dataset_dir,f'FFHQ_LPFF.json'), "w", newline='\n') as f:
        f.write(data)
    ...
    plt.scatter(
        thetas_lpff/ np.pi * 180, phis_lpff / np.pi * 180,  # 纵坐标
    ...
    # 显示所绘图形
    plt.savefig('./imgs/lpff.png', dpi=100, format='png',
                bbox_inches='tight')
    plt.savefig('./imgs/lpff.pdf', dpi=100, format='pdf',
                bbox_inches='tight')
    plt.show()
