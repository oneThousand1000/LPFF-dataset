...
def convert_dataset(
    ...
    for json_file_path in file_list:
        ...
        if  os.path.basename(json_file_path).split('.')[0] == 'dataset':
            continue
        file_name = os.path.basename(json_file_path)
        print(f'os.path.isfile({json_file_path}): ', os.path.isfile(json_file_path))
        if os.path.isfile(json_file_path):
            print('write ',json_file_path)
            with open(json_file_path, 'r') as file:
                metadata = json.load(file)
                save_bytes(os.path.join(archive_root_dir, file_name), json.dumps(metadata))
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        #print('archive_fname: ',archive_fname)
